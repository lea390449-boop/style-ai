...
export const generateLook = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const promptText = [
      ...
      `BODY INCLUSIVITY & PROPORTION ACCURACY — render the body at the SAME true size, shape, height, and proportions as the reference person (${notes}). Do NOT slim, stretch, elongate, idealize, or push toward a "model" body type. Honor every body: petite, tall, plus-size, curvy, straight, athletic, apple, pear, hourglass, broad-shouldered, narrow, soft, muscular — all rendered with dignity, accurate volume, and natural posture.`,
      `ANATOMICAL PROPORTIONS — the figure must be anatomically correct and human-proportionate. Use realistic head-to-body ratio (roughly 7–8 heads tall for adults, adjusted to the person's actual height). Shoulders, torso length, arm length (fingertips at mid-thigh when relaxed), waist placement, hip width, leg length (legs ≈ half of total height), hand size (≈ face length), and foot size must all be true to real human anatomy and consistent with each other. No elongated legs, no shrunken heads, no oversized hands, no doll-like or fashion-illustration distortions. Left and right sides of the body must be symmetric in size. Garments should drape realistically over the actual body, not a thinner or stretched stand-in.`,
